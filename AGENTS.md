...
- 코드 수정 후 관련 테스트를 Codex가 직접 실행한다.
- 불필요한 재확인 없이, 현재 승인 정책 범위 내에서 즉시 수행한다.
- 테스트 실행 결과를 반드시 확인하고 보고한다.
...
- /usr/local/bin/php83 artisan test
- /usr/local/bin/php83 artisan test --filter=TestName
- /usr/local/bin/php83 vendor/bin/phpunit
- /usr/local/bin/php83 vendor/bin/phpunit --filter=TestName

---

### 필수 규칙

- php, composer 명령어 직접 사용 금지
- 반드시 /usr/local/bin/php83 사용
- 테스트 없이 완료 처리 금지
- 실패한 테스트를 무시하거나 건너뛰기 금지
